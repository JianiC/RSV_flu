...
give_fit_trajectories <- function(counter = 3, model_params = simulate_from) {
  ...
  # Create pomp model
  TX_data_3s<-read.csv("test_TX_data_18_20.csv")  
  # Create pomp model
...
## simulate trajectory end
###########################################################################
##########################################################################
load("cluster_result/old_3s/test_result_neutral.Rdata")
as_tibble(t(c(result_neutral$GAobj@solution[,3:8], 
  ...
  mutate(logLik = result_neutral$GAobj@fitnessValue,
         AIC = calculate_aic(logLik, npar = 8 )) -> TX_3s_null_old
...
TX_data_3s %>% 
  ...
  bind_cols(all_fit) %>% 
  select(2, 3, 1, 4:7) -> all_fit_with_data_old
...
all_fit_with_data_old %>% 
  filter(Model == "Nulll") %>% 
  filter(flu_type == "scases_A") -> all_fit_with_data_old_subset1


# make the plot_object 

all_fit_with_data_old_subset1 %>% 
