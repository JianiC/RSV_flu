...
rproc_euler_multinomial <- Csnippet("
  ...
  rate[7] = chi_AB * foiB;          // infectious of CA_SB to RA_IB
  ...
  rate[9] = chi_BA * foiA;          // infectious of SA_CB to IA_RB
  ...
  rate[16] = w_A;       // loss of immunity from RA_RB to SA_RB
  rate[17] = w_B ;      // loss of immunity from RA_RB to RA_SB
...
det_skel <- Csnippet("
  ...
  rate[7] = CA_SB*chi_AB * foiB;          // infectious of CA_SB to RA_IB
  rate[8] = SA_CB*phi_B;                    //  loss immunity of SA_CB to SA_RB
  rate[9] = SA_CB*chi_BA * foiA;          // infectious of SA_CB to IA_RB
  ...
  rate[16] = RA_RB*w_A;       // loss of immunity from RA_RB to SA_RB
  rate[17] = RA_RB*w_B;       // loss of immunity from RA_RB to RA_SB
...
data.frame(time = seq(0.5, 1.5, by = 1/52), 
           total_a = NA, 
           total_b = NA) %>% 
...
simulate_tss <- function(params, give_everything = FALSE, show_progress = TRUE,...) {
  ...
  guess1_params <- c(R0_A=unname(params[,"R0"]), gamma_A=365./9, w_A=1./1.,
                     ...
                     lamda_BA=unname(params[,"lamda"]), lamda_AB=unname(params[,"lamda"]), 
  ...
  pomp_sirs %>%
    ...
    mutate(mp_A = max(K_A), 
           ...
           pw_A = which(K_A == max(K_A)), 
           pw_B = which(K_B == max(K_B)), 
           p_AB_diff = pw_A - pw_B, 
           pop = guess1_all["pop"],
...
###########################################################################
params_design1 <- expand.grid(chi = 1, 
                              rel = seq(0.5, 2, length = 500), 
                              lamda = 1,
                              R0 = 2, 
                              phi = 0)

pb <- progress_estimated(nrow(params_design1))
tic()
library(future)
library(furrr)
plan(multisession)
res_df <- future_map_dfr(.x = 1:nrow(params_design3), .f = multi_simulate_tss,
                         params_mat = params_design3, .progress = TRUE)
toc()

lamda_design<-params_design1 %>%
  bind_cols(res_df)
...
params_design2 <- expand.grid(chi = seq(1e-10, 1, length = 500), 
                             rel = 1, 
                             lamda =1 ,
                             R0 = 2, 
                             phi = 365/30)

params_design3 <- expand.grid(chi = 1, 
                              rel = 0.8, 
                              lamda = seq(1e-10, 1, length = 500),
                              R0 = 2, 
                              phi = 365/30)