...
df_restim_IAVHI = df_restim_IAVHI[!duplicated(df_restim_IAVHI$`Mean(R)`),]
...
df_restim_IAVH3 = df_restim_IAVH3[!duplicated(df_restim_IAVH3$`Mean(R)`),]
...
df_restim_B_vic = df_restim_B_vic[!duplicated(df_restim_B_vic$`Mean(R)`),]
...
df_restim_B_yam = df_restim_B_yam[!duplicated(df_restim_B_yam$`Mean(R)`),]
...
For both RSVA and RSVB the mean si was changed to 4.5 and std_si was changed to 0.894. (Cite: https://www.medrxiv.org/content/10.1101/2020.02.04.20020404v2.full?fbclid=IwAR1wsG0Bgr0h0lckr0PNIETlU47rIEfJXpwDdkNdyNRAbXjhG74mpCfqau)
...
df_restim_RSVA = df_restim_RSVA[!duplicated(df_restim_RSVA$`Mean(R)`),]
...
df_restim_RSVB = df_restim_RSVB[!duplicated(df_restim_RSVB$`Mean(R)`),]
