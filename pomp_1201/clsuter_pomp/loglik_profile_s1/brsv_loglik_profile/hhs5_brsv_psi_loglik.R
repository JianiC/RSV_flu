...
best_past_est<-data.frame(past_est(res_hhs5_brsv_coinfect)$R01,
                          past_est(res_hhs5_brsv_coinfect)$R02,
                          past_est(res_hhs5_brsv_coinfect)$amplitude1,
                          past_est(res_hhs5_brsv_coinfect)$amplitude2,
                          past_est(res_hhs5_brsv_coinfect)$tpeak1,
                          past_est(res_hhs5_brsv_coinfect)$tpeak2,
                          past_est(res_hhs5_brsv_coinfect)$rho1,
                          past_est(res_hhs5_brsv_coinfect)$rho2)

psi_vals<-c(seq(0,1,by=0.1),past_est(res_hhs5_brsv_coinfect)$psi)

## loop different psi values
res_hhs5_brsv_psi<-data.frame()
...
for (i in 1:length(psi_vals)){
  ...
  print(psi_vals[i])
  print(res_hhs5_brsv_psi_tmp$DEobj$optim$bestval)
  
  res_hhs5_brsv_psi_tmp <- data.frame (HHS_region =5,
                                       pathogen2 = "fluB",
                                       psi=psi_vals[i],
                                       loglik=-res_hhs5_brsv_psi_tmp$DEobj$optim$bestva)
  res_hhs5_brsv_psi <-rbind(res_hhs5_brsv_psi,res_hhs5_brsv_psi_tmp)
  
}

write.csv(res_hhs5_brsv_psi,"res_hhs5_brsv_psi_loglik.csv",row.names=FALSE)
