...
psi_vals<-c(seq(0,1,by=0.1),past_est(res_hhs4_brsv_coinfect)$psi)
...
for (i in 1:length(psi_vals)){
  ...
  res_hhs4_brsv_psi_tmp <- data.frame (HHS_region =4,
                                       pathogen2 = "fluB",
                                       psi=psi_vals[i],
                                       loglik=-res_hhs4_brsv_psi_tmp$DEobj$optim$bestva)
  res_hhs4_brsv_psi <-rbind(res_hhs4_brsv_psi,res_hhs4_brsv_psi_tmp)
  
}

write.csv(res_hhs4_brsv_psi,"res_hhs4_brsv_psi_loglik.csv",row.names=FALSE)
