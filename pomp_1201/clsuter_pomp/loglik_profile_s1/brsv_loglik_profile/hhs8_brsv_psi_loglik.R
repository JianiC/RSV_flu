...
## from estimation
load("./res_hhs8_brsv_coinfect.rds")
...
psi_vals<-c(seq(0,1,by=0.1),past_est(res_hhs8_brsv_coinfect)$psi)

## loop different psi values
res_hhs8_brsv_psi<-data.frame()
...
for (i in 1:length(psi_vals)){
  ...
  print(psi_vals[i])
  print(res_hhs8_brsv_psi_tmp$DEobj$optim$bestval)
  
  res_hhs8_brsv_psi_tmp <- data.frame (HHS_region =8,
                                       pathogen2 = "fluB",
                                       psi=psi_vals[i],
                                       loglik=-res_hhs8_brsv_psi_tmp$DEobj$optim$bestva)
  res_hhs8_brsv_psi <-rbind(res_hhs8_brsv_psi,res_hhs8_brsv_psi_tmp)
  
}

write.csv(res_hhs8_brsv_psi,"res_hhs8_brsv_psi_loglik.csv",row.names=FALSE)
