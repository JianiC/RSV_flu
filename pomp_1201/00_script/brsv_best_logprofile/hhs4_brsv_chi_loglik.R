...
best_past_est<-c(R01=past_est(res_hhs4_brsv_coinfect)$R01,
                          ...
                          psi=past_est(res_hhs4_brsv_coinfect)$psi)

chi_vals<-seq(0,1,by=0.1)

## loop different chi values
res_hhs4_brsv_chi<-data.frame()
...
for (i in 1:length(chi_vals)){
  ...
  if(res_hhs4_brsv_chi_tmp$total2 == "fluB") message("Code itegration complete!")
  print(chi_vals[i])
  print(res_hhs4_brsv_chi_tmp$DEobj$optim$bestval)
  
  res_hhs4_brsv_chi_tmp <- data.frame (HHS_region =4,
                                       pathogen2 = "fluB",
                                       chi=chi_vals[i],
                                       loglik=-res_hhs4_brsv_chi_tmp$DEobj$optim$bestva)
  res_hhs4_brsv_chi <-rbind(res_hhs4_brsv_chi,res_hhs4_brsv_chi_tmp)
  
}

write.csv(res_hhs4_brsv_chi,"res_hhs4_brsv_chi_loglik_s2.csv",row.names=FALSE)
