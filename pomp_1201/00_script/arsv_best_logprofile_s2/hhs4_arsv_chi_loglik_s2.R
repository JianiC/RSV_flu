...
for (i in 1:length(chi_vals)){
  ...
  print(chi_vals[i])
  print(res_hhs4_arsv_chi_tmp$DEobj$optim$bestval)
  
  res_hhs4_arsv_chi_tmp <- data.frame (HHS_region =4,
                                       pathogen2 = "fluA",
                                       chi=chi_vals[i],
                                       loglik=-res_hhs4_arsv_chi_tmp$DEobj$optim$bestva)
  res_hhs4_arsv_chi <-rbind(res_hhs4_arsv_chi,res_hhs4_arsv_chi_tmp)
  
}

write.csv(res_hhs4_arsv_chi,"res_hhs4_arsv_chi_loglik_s2.csv",row.names=FALSE)
