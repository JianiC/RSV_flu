...
## from estimation
load("./res_hhs5_arsv_coinfect.rds")

best_past_est<-c(R01=past_est(res_hhs5_arsv_coinfect)$R01,
                          R02=past_est(res_hhs5_arsv_coinfect)$R02,
                          amplitude1=past_est(res_hhs5_arsv_coinfect)$amplitude1,
                          amplitude2 =past_est(res_hhs5_arsv_coinfect)$amplitude2,
                          tpeak1=past_est(res_hhs5_arsv_coinfect)$tpeak1,
                          tpeak2=past_est(res_hhs5_arsv_coinfect)$tpeak2,
                          rho1=past_est(res_hhs5_arsv_coinfect)$rho1,
                          rho2=past_est(res_hhs5_arsv_coinfect)$rho2,
                          psi=past_est(res_hhs5_arsv_coinfect)$psi)
                          
chi_mle<-past_est(res_hhs5_arsv_coinfect)$chi
...
for (i in 1:length(chi_vals)){
  ...
  print(chi_vals[i])
  print(res_hhs5_arsv_chi_tmp$DEobj$optim$bestval)
  
  res_hhs5_arsv_chi_tmp <- data.frame (HHS_region =5,
                                       pathogen2 = "fluA",
                                       chi=chi_vals[i],
                                       loglik=-res_hhs5_arsv_chi_tmp$DEobj$optim$bestva)
  res_hhs5_arsv_chi <-rbind(res_hhs5_arsv_chi,res_hhs5_arsv_chi_tmp)
  
}

write.csv(res_hhs5_arsv_chi,"res_hhs5_arsv_chi_loglik_s2.csv",row.names=FALSE)
