...
best_past_est<-c(R01=past_est(res_hhs3_arsv_coinfect)$R01,
                          ...
                          chi=past_est(res_hhs3_arsv_coinfect)$chi)
...
for (i in 1:length(psi_vals)){
  ...
  res_hhs3_arsv_psi_tmp <- data.frame (HHS_region =3,
                                       pathogen2 = "fluA",
                                       psi=psi_vals[i],
                                       loglik=-res_hhs3_arsv_psi_tmp$DEobj$optim$bestval)
  res_hhs3_arsv_psi <-rbind(res_hhs3_arsv_psi,res_hhs3_arsv_psi_tmp)
  
}

write.csv(res_hhs3_arsv_psi,"res_hhs3_arsv_psi_loglik_s3.csv",row.names=FALSE)
