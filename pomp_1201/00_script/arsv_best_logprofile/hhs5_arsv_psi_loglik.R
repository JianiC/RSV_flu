...
best_past_est<-c(R01=past_est(res_hhs5_arsv_coinfect)$R01,
                          ...
                          chi=past_est(res_hhs5_arsv_coinfect)$chi)

psi_vals<-seq(0,1,by=0.1)

## loop different chi values
res_hhs5_arsv_psi<-data.frame()
...
for (i in 1:length(psi_vals)){
  ...
  print(psi_vals[i])
  print(res_hhs5_arsv_psi_tmp$DEobj$optim$bestval)
  
  res_hhs5_arsv_psi_tmp <- data.frame (HHS_region =5,
                                       pathogen2 = "fluA",
                                       psi=psi_vals[i],
                                       loglik=-res_hhs5_arsv_psi_tmp$DEobj$optim$bestval)
  res_hhs5_arsv_psi <-rbind(res_hhs5_arsv_psi,res_hhs5_arsv_psi_tmp)
  
}

write.csv(res_hhs5_arsv_psi,"res_hhs5_arsv_psi_loglik_s3.csv",row.names=FALSE)
