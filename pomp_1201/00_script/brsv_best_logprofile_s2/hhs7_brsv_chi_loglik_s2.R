...
best_past_est<-c(R01=past_est(res_hhs7_brsv_coinfect)$R01,
                          R02=past_est(res_hhs7_brsv_coinfect)$R02,
                          amplitude1=past_est(res_hhs7_brsv_coinfect)$amplitude1,
                          amplitude2 =past_est(res_hhs7_brsv_coinfect)$amplitude2,
                          tpeak1=past_est(res_hhs7_brsv_coinfect)$tpeak1,
                          tpeak2=past_est(res_hhs7_brsv_coinfect)$tpeak2,
                          rho1=past_est(res_hhs7_brsv_coinfect)$rho1,
                          rho2=past_est(res_hhs7_brsv_coinfect)$rho2,
                          psi=past_est(res_hhs7_brsv_coinfect)$psi)

chi_mle<-past_est(res_hhs7_brsv_coinfect)$chi
chi_mle_low <- 0.75*chi_mle
chi_mle_up <- 1.25*chi_mle
chi_vals<-seq(chi_mle_low,chi_mle_up,by=0.03)

## loop different chi values
res_hhs7_brsv_chi<-data.frame()
...
for (i in 1:length(chi_vals)){
  ...
  print(chi_vals[i])
  print(res_hhs7_brsv_chi_tmp$DEobj$optim$bestval)
  
  res_hhs7_brsv_chi_tmp <- data.frame (HHS_region =7,
                                       pathogen2 = "fluB",
                                       chi=chi_vals[i],
                                       loglik=-res_hhs7_brsv_chi_tmp$DEobj$optim$bestva)
  res_hhs7_brsv_chi <-rbind(res_hhs7_brsv_chi,res_hhs7_brsv_chi_tmp)
  
}

write.csv(res_hhs7_brsv_chi,"res_hhs7_brsv_chi_loglik_s2.csv",row.names=FALSE)
