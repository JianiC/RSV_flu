...
psi_vals<-c(seq(0,1,by=0.1),past_est(res_hhs5_arsv_coinfect)$psi)
...
for (i in 1:length(psi_vals)){
  ...
  res_hhs5_arsv_psi_tmp <- data.frame (HHS_region =5,
                                       pathogen2 = "fluA",
                                       psi=psi_vals[i],
                                       loglik=-res_hhs5_arsv_psi_tmp$DEobj$optim$bestva)
  res_hhs5_arsv_psi <-rbind(res_hhs5_arsv_psi,res_hhs5_arsv_psi_tmp)
  
}

write.csv(res_hhs5_arsv_psi,"res_hhs5_arsv_psi_loglik.csv",row.names=FALSE)
