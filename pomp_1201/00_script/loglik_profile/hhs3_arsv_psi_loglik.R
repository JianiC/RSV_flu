...
psi_vals<-c(seq(0,1,by=0.1),past_est(res_hhs3_arsv_coinfect)$psi)

## loop different psi values
res_hhs3_arsv_psi<-data.frame()
...
for (i in 1:length(psi_vals)){
  ...
  print(psi_vals[i])
  print(res_hhs3_arsv_psi_tmp$DEobj$optim$bestval)
  
  res_hhs3_arsv_psi_tmp <- data.frame (HHS_region =3,
                                       pathogen2 = "fluA",
                                       psi=psi_vals[i],
                                       loglik=-res_hhs3_arsv_psi_tmp$DEobj$optim$bestva)
  res_hhs3_arsv_psi <-rbind(res_hhs3_arsv_psi,res_hhs3_arsv_psi_tmp)
  
}

write.csv(res_hhs3_arsv_psi,"res_hhs3_arsv_psi_loglik.csv",row.names=FALSE)
