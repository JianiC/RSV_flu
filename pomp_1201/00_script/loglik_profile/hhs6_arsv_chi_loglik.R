...
chi_vals<-c(seq(0,1,by=0.1),past_est(res_hhs6_arsv_coinfect)$chi)

## loop different chi values
res_hhs6_arsv_chi<-data.frame()
...
for (i in 1:length(chi_vals)){
  ...
  print(chi_vals[i])
  print(res_hhs6_arsv_chi_tmp$DEobj$optim$bestval)
  
  res_hhs6_arsv_chi_tmp <- data.frame (HHS_region =6,
                                       pathogen2 = "fluA",
                                       chi=chi_vals[i],
                                       loglik=-res_hhs6_arsv_chi_tmp$DEobj$optim$bestva)
  res_hhs6_arsv_chi <-rbind(res_hhs6_arsv_chi,res_hhs6_arsv_chi_tmp)
  
}

write.csv(res_hhs6_arsv_chi,"res_hhs6_arsv_chi_loglik.csv",row.names=FALSE)
