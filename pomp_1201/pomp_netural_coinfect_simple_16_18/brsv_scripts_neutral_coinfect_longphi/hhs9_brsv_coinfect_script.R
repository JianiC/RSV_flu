...
if(FALSE) {
  ...
  pseudo_data <- tibble(time = seq(0, 10, by = 1/52), 
                        total1 = NA, 
                        total2 = NA, N = pomp_data_hhs9_brsv$N[1])
  
  # make a pomp object 
  hhs9_a_rsv_po <- make_pomp(df = pseudo_data, time_start_sim = -100)
  # inspect the compiled pomp object
  # spy(hhs9_a_rsv_po)
  
# test if the integratro works as default
test_traj <- trajectory(object = hhs9_a_rsv_po, format = "d", method = "ode23")
...
save(res_hhs9_brsv_coinfect, file = "../testb_result_coinfect/res_hhs9_brsv_coinfect.rds")
