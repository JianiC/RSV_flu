...
#chi_mle<-past_est(res_hhs6_arsv_coinfect)$chi
psi_mle_low <- 0.01
psi_mle_up <- 0.29
...
for (i in 1:length(psi_vals)){
  ...
  res_hhs6_arsv_psi_tmp <- data.frame (HHS_region =6,
                                       pathogen2 = "fluA",
                                       psi=psi_vals[i],
                                       loglik=-res_hhs6_arsv_psi_tmp$DEobj$optim$bestval)
  res_hhs6_arsv_psi <-rbind(res_hhs6_arsv_psi,res_hhs6_arsv_psi_tmp)
  
}

write.csv(res_hhs6_arsv_psi,"res_hhs6_arsv_psi_loglik_s2.csv",row.names=FALSE)
